...
function main(){
    ...
    for(let alunos of lista_alunos){
        ...
        }else if(!aprovado(nota1,nota2,nota3)){
            reprovados++
        }
        if(prova(nota1,nota2,nota3)){
            prova_final++
            reprovados--
        }
        ...
        aluno++
        console.log(`Media do aluno ${nome}: ${media(nota1,nota2,nota3).toFixed(1)}`)
    }   
    console.log(`Media classe: ${media_classe(soma_media,aluno).toFixed(1)}`)
...
function loadfile(){
    try{
        const data  = fs.readFileSync('dados_alunos.txt','utf-8')
        return data
    }catch(error){
        console.error(error)
    }
